...
public class Board5Utils
{
    ...
    /// <returns></returns>
    public static double EvaluateBoard(Connect5Board connect5Board, PlayerAlliance alliance)
    {
        if (Connect5Utils.Finished(connect5Board))
        {
            return Connect5Utils.INF;
        }
        ...
        for (int i = 0; i < NUM_ROWS; i++)
        {
            for (int j = 0; j < NUM_COLS; j++)
            {
                if (connect5Board.Table[i, j] != Tile.EMPTY)
                {
                    if (connect5Board.Table[i, j] == Tile.RED && alliance == PlayerAlliance.RED)
                    {
                        score += evaluationBoard[i, j];
                    }
                    else if (connect5Board.Table[i, j] == Tile.BLACK && alliance == PlayerAlliance.BLACK)
                    {
                        score += evaluationBoard[i, j];
                    }
                }
